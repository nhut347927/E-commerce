...
public class EmailServiceImpl implements IEmailService {
    ...
    /**
     * Sends a contact form email from formEmail to formEmail with the user's name, email, and message.
     *
    ...
    public void sendContactFormEmail(String name, String email, String message) {
        validateContactInputs(name, email, message);
        String subject = appName + " - New Contact Form Submission";
        String emailContent = generateContactEmailContent(name, email, message);
        ...
        try {
            ...
            throw new AppException("Failed to send contact form email: " + e.getMessage(), 500);
        }
    }
    ...
    private void validateContactInputs(String name, String email, String message) {
        if (name == null || name.isBlank()) {
            throw new AppException("Name must not be null or empty", 400);
        }
        if (email == null || email.isBlank()) {
            throw new AppException("Email must not be null or empty", 400);
        }
        ...
        if (message == null || message.isBlank()) {
            throw new AppException("Message must not be null or empty", 400);
        }
    }
     ...
     * @return HTML email content
     */
    private String generateContactEmailContent(String name, String email, String message) {
        return """
                <!DOCTYPE html>
                <html>
                <head>
                    ...
                    <style>
                        ...
                        .field-label { font-weight: bold; color: #333333; }
                        .footer { font-size: 12px; color: #777777; text-align: center; margin-top: 20px; }
                        @media only screen and (max-width: 600px) {
                            .container { padding: 15px; }
                        }
                ...
                <body>
                    <div class="container">
                        ...
                        <p><span class="field-label">Name:</span> %s</p>
                        <p><span class="field-label">Email:</span> %s</p>
                ...
                """
                .formatted(name, email, message, appName, appName.toLowerCase(), appName.toLowerCase());
    }
}