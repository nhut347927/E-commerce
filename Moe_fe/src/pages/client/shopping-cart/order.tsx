  ...
  const handleSubmitRating = async (orderItemCode: string) => {
    ...
    try {
      ...
      if (response.data.code === 200) {
        ...
        if (orderItemsData) {
          const updatedItems = orderItemsData.map((item) =>
      ...
      <div className="py-8 bg-gray-100 mb-20">
        <div className="max-w-7xl w-full mx-auto px-3 sm:px-16">
          <div className="flex flex-col items-start">
            <h4 className="text-2xl font-semibold text-gray-800">Your Orders</h4>
      ...
      <div className="max-w-7xl w-full mx-auto px-3 sm:px-16">
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Select onValueChange={handleSizeChange} defaultValue={filter.size.toString()}>
              <SelectTrigger className="h-10 w-32 rounded-none">
        ...
        {!ordersLoading && !ordersError && ordersData?.contents.length === 0 && (
          <div className="py-8 text-center text-gray-600">You have no orders.</div>
        )}

        {!ordersLoading && !ordersError && ordersData && ordersData?.contents.length > 0 && (
            ...
            {ordersData?.contents.map((order) => (
              <div key={order.code} className="border-b border-gray-200 py-4">
                <div className="grid grid-cols-5 gap-4 items-center">
                  ...
                  <div className="text-gray-600">{formatDate(order.createAt)}</div>
                  <div className="text-gray-800 font-semibold">{formatVnPrice(order.total)}</div>
                  <div className="text-gray-600">{order.deliveryStatus}</div>
                  <Button
                    variant="ghost"
                ...
                {expandedOrder === order.code && (
                  <div className="mt-4">
                    {itemsLoading && (
                      <div className="py-4 text-center text-gray-600">Loading details...</div>
                    )}
                    {itemsError && (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                Quantity: {item.quantity} | Price: {formatVnPrice(item.price)}
                              </p>
                              {item.rating ? (
                                <p className="text-sm text-green-600 mt-1">Rated</p>
                              ) : (
                                <div className="mt-2">
                                  <div className="flex flex-col gap-2">
                                      ...
                                      {[1, 2, 3, 4, 5].map((star) => (
                                        <Star
                                          ...
                                          className={`h-6 w-6 cursor-pointer ${
                                            star <= (ratingForm[item.code]?.ratingValue || 0)
                                              ? 'fill-yellow-400 stroke-yellow-400'
                                              : 'stroke-gray-300'
                                          }`}
                                          onClick={() => handleStarClick(item.code, star)}
                                    ...
                                    <Input
                                      type="text"
                                      value={ratingForm[item.code]?.comment || ''}
                                      onChange={(e) =>
                                        setRatingForm((prev) => ({
                                          ...prev,
                                          [item.code]: {
                                    ...
                                    />
                                    <Button
                                      onClick={() => handleSubmitRating(item.code)}
