...
const CategoryPage: React.FC = () => {
  ...
  const { data, loading, error, refetch } = useGetApi<Page<CategoryAll>>({
    endpoint: "/category",
  ...
  const handleCreateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosInstance.post("/category", createFormData);
  ...
  const handleUpdateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosInstance.put("/category", updateFormData);
      if (response.data.code === 200) {
        toast({
          title: "Success",
  ...
  const handleOpenUpdateDialog = (category: CategoryAll) => {
    setUpdateFormData({ code: category.code, name: category.name });
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              data.contents.map((category, index) => (
                <TableRow key={category.code}>
                  <TableCell>{index + 1}</TableCell>
                  <TableCell>{category.name}</TableCell>
                  <TableCell>{category.userCreateDisplayName}</TableCell>
                  <TableCell>{formatDateTime(category.createAt)}</TableCell>
                  <TableCell>{category.userUpdateDisplayName || "-"}</TableCell>
                  <TableCell>{formatDateTime(category.updateAt)}</TableCell>
