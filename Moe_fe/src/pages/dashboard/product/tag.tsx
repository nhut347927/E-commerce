...
const TagPage: React.FC = () => {
  ...
  const { data, loading, error, refetch } = useGetApi<Page<TagAll>>({
    endpoint: "/tag",
  ...
  const handleCreateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosInstance.post("/tag", createFormData);
  ...
  const handleUpdateSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axiosInstance.put("/tag", updateFormData);
      if (response.data.code === 200) {
        toast({
          title: "Success",
      ...
      toast({
        ...
        description:
          err.response?.data?.message ||
          "An error occurred while updating tag.",
        variant: "destructive",
      });
    }
  };
  ...
  const handleDelete = async (code: string) => {
    try {
      const response = await axiosInstance.delete("/tag", { data: { code } });
      if (response.data.code === 200) {
        toast({
          title: "Success",
  ...
  const handleOpenUpdateDialog = (tag: TagAll) => {
    setUpdateFormData({ code: tag.code, name: tag.name });
    setIsUpdateDialogOpen(true);
  };

  return (
    <div className="">
      <h2 className="text-2xl font-bold text-gray-800 mb-6">Tag Management</h2>
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              data.contents.map((tag, index) => (
                <TableRow key={tag.code}>
                   <TableCell>{index+1}</TableCell>
                  <TableCell><span className="border-2 hover:bg-black hover:text-white p-2 px-4 rounded-full">{tag.name}</span></TableCell>
                  <TableCell>{tag.userCreateDisplayName}</TableCell>
                  <TableCell>{formatDateTime(tag.createAt)}</TableCell>
                  <TableCell>{tag.userUpdateDisplayName || "-"}</TableCell>
                  <TableCell>{formatDateTime(tag.updateAt)}</TableCell>
                  <TableCell className="flex space-x-2">
                    <Button
                      variant="outline"
                      size="icon"
                      onClick={() => handleOpenUpdateDialog(tag)}
                      className="border-gray-300 text-gray-600 hover:text-zinc-500"
                    >
