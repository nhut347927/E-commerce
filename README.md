...
- Node.js >= 18 (cho FE)
- Java JDK 17 (cho BE)
- MySQL Server (cho production DB)
- Tài khoản Cloudinary, VNPay, Google OAuth (cấu hình trong .env)

### Cài Đặt Frontend
1. Clone repository: `git clone <repo-url>`
2. Chuyển đến thư mục FE: `cd frontend`
3. Cài đặt dependencies: `npm install` hoặc `yarn install`
4. Tạo file `.env` dựa trên `.env.example` (cấu hình API base URL, v.v.)
5. Chạy dev server: `npm run dev` hoặc `yarn dev`

### Cài Đặt Backend
1. Chuyển đến thư mục BE: `cd backend`
2. Cài đặt dependencies: `mvn clean install`
3. Tạo file `.env` hoặc cấu hình trong `application.properties` (DB URL, JWT secret, Cloudinary API key, v.v.)
4. Chạy server: `mvn spring-boot:run`

### Database Setup
- Sử dụng script SQL trong `/database/schema.sql` để tạo DB.
- H2 cho dev/test, MySQL cho production.
- Chạy migration nếu cần (tích hợp Flyway nếu mở rộng).

## Hướng Dẫn Chạy Dự Án
- Chạy BE trước để API sẵn sàng.
- Truy cập FE tại `http://localhost:5173` (default Vite port).
- Đăng nhập admin với tài khoản default (nếu có) hoặc tạo mới.
...
│   ├── pages/          # Các trang (Home, Product, etc.)
│   ├── store/          # Redux store
